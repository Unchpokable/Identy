...
TEST_F(VMDetectionTest, AssumeVirtual_MatchesExpectedEnvironment)
{
    ...
    if constexpr(kExpectVirtualMachine) {
        EXPECT_TRUE(is_vm) << "Expected VM detection in CI/cloud environment. "
                           << "If running locally, use -DIDENTY_TEST_EXPECT_BAREMETAL=ON";
    }
    else {
        EXPECT_FALSE(is_vm) << "Expected bare metal detection on local machine. "
...
TEST_F(VMDetectionTest, AssumeVirtualEx_MatchesExpectedEnvironment)
{
    ...
    if constexpr(kExpectVirtualMachine) {
        EXPECT_TRUE(is_vm) << "Expected VM detection (extended) in CI/cloud environment";
    }
    else {
        EXPECT_FALSE(is_vm) << "Expected bare metal detection (extended) on local machine";
    }
}
...
TEST_F(VMDetectionTest, AnalyzeFull_ReturnsValidVerdict)
{
    ...
    EXPECT_GE(static_cast<int>(verdict.confidence), static_cast<int>(vm::VMConfidence::Unlikely));
    EXPECT_LE(static_cast<int>(verdict.confidence), static_cast<int>(vm::VMConfidence::DefinitelyVM));
}
...
TEST_F(VMDetectionTest, AnalyzeFull_ConfidenceMatchesEnvironment)
{
    ...
    if constexpr(kExpectVirtualMachine) {
        EXPECT_GE(verdict.confidence, vm::VMConfidence::Probable) << "In VM environment, confidence should be at least Probable";
    }
    else {
        EXPECT_LE(verdict.confidence, vm::VMConfidence::Possible) << "On bare metal, confidence should be at most Possible";
    }
}
...
TEST_F(VMDetectionTest, AnalyzeFull_DetectionFlagsInVM)
{
    if constexpr(kExpectVirtualMachine) {
        auto verdict = identy::vm::analyze_full(mb_);
        EXPECT_FALSE(verdict.detections.empty()) << "In VM environment, at least one detection flag should be set";
    }
}
...
TEST_F(VMDetectionTest, HeuristicVerdict_IsVirtualConsistentWithConfidence)
{
    ...
    EXPECT_EQ(is_virtual, confidence_indicates_vm) << "is_virtual() should be true iff confidence >= Probable";
}
...
TEST_F(VMDetectionTest, HeuristicVerdict_DetectionsMatchConfidence)
{
    ...
    if(verdict.confidence == vm::VMConfidence::DefinitelyVM) {
        EXPECT_FALSE(verdict.detections.empty()) << "DefinitelyVM confidence requires at least one detection flag";
    }

    // If no detections, confidence should be Unlikely
    if(verdict.detections.empty()) {
        EXPECT_EQ(verdict.confidence, vm::VMConfidence::Unlikely) << "Empty detections should result in Unlikely confidence";
    }
}
...
TEST(DefaultWeightPolicyTest, GetStrength_AllFlagsHandled)
{
    ...
    for(auto flag : all_flags) {
        ...
        EXPECT_GE(static_cast<int>(strength), static_cast<int>(vm::detail::FlagStrength::Weak));
        EXPECT_LE(static_cast<int>(strength), static_cast<int>(vm::detail::FlagStrength::Critical));
    }
}
...
static_assert(vm::WeightPolicy<TestWeightPolicy>, "TestWeightPolicy should satisfy WeightPolicy concept");
static_assert(vm::WeightPolicy<vm::DefaultWeightPolicy>, "DefaultWeightPolicy should satisfy WeightPolicy concept");
...
TEST(CustomWeightPolicyTest, CustomPolicy_AcceptsCutomPolicy)
{
}
...
static_assert(vm::Heuristic<vm::DefaultHeuristic<>>, "DefaultHeuristic should satisfy Heuristic concept");
static_assert(vm::HeuristicEx<vm::DefaultHeuristicEx<>>, "DefaultHeuristicEx should satisfy HeuristicEx concept");
...
TEST_F(VMDetectionTest, AnalyzeFull_Deterministic)
{
    ...
    EXPECT_EQ(verdict1.confidence, verdict2.confidence) << "analyze_full confidence should be deterministic";
    EXPECT_EQ(verdict1.detections.size(), verdict2.detections.size()) << "analyze_full detections count should be deterministic";
}
...
TEST_F(VMDetectionTest, ExtendedVsBasic_ConsistentDirection)
{
    ...
    EXPECT_EQ(verdict_basic.is_virtual(), verdict_ex.is_virtual()) << "Basic and extended analysis should agree on VM detection";
}
